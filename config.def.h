...
static Key keys[] = {
  ...
  { ShiftMask,											XK_Print,			spawn,					SHCMD("maim -f jpg -m 9 -s \"/home/flex/Pictures/SCREENSHOTS/$(date +\"%F %H_%M_%S.jpg\")\"")},
	...
	{ MODKEY|ShiftMask,								XK_e,					spawn,					SHCMD("cd ~/.local/src/dwm/; kitty -T DWMconfig nvim config.def.h")},
...
// dwm bindings
  ...
  { MODKEY|ControlMask,							XK_k,					focusmon,				{.i = -1 } },
	...
	{ MODKEY|ShiftMask,     	        XK_Return,		spawn,   	      SHCMD("tmux send-keys -t main clear; kitty -T Tmux tmux new-session -At main") },
