...
static Key keys[] = {
  ...
  { ShiftMask,											XK_Print,			spawn,					SHCMD("maim -f jpg -m 9 -s \"/home/flex/Pictures/SCREENSHOTS/$(date +\"%F %H_%M_%S.jpg\")\"")},
	...
	{ 0,						XF86XK_MonBrightnessUp,					spawn,			    SHCMD("n=$(brightnessctl | awk '/Current/' | sed 's/.*(\\([^%]*\\).*/\\1/'); brightnessctl set $((n+n/2+1))%") },
	{ 0,						XF86XK_MonBrightnessDown,				spawn,			    SHCMD("n=$(brightnessctl | awk '/Current/' | sed 's/.*(\\([^%]*\\).*/\\1/'); brightnessctl set $((n-n/2+1))%") },
